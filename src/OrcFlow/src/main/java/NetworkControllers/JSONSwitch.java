...
public class JSONSwitch {
    ...
    public ArrayList < SwitchData > switches_RYU(ArrayList < LinkExtData > arrayLED, ControlData controller) throws ParseException, URISyntaxException {
    ...
    throws ParseException, URISyntaxException {
        ...
        try {
            ...
            for (int i = 0; i < arraySwitches.length(); i++) {
                ...
                portsDescs = new JSONObject(pD);
                arrayPorts = portsDescs.getJSONArray("port_desc");

                for (int j = 0; j < arrayPorts.length(); j++) {
                    port = arrayPorts.getJSONObject(j);
                    portNumber = port.getString("port_number");
                    name = port.getString("name");
                    ...
                    if (name.contains("ext")) {
                        LinkExtData linkExt = new LinkExtData();
                        linkExt.setDPID(swD.getDPID());
                        linkExt.setPort(port.getString("port_number"));
                        int pos = name.indexOf("ext") + 3;
    ...
    public ArrayList < SwitchData > switches_OPENDAYLIGHT(
        ArrayList < LinkExtData > arrayLED,
        ControlData controller
    ) throws
    ParseException,
    URISyntaxException,
    IOException {
        ...
        try {
            ...
            for (int i = 0; i < nodes.length(); i++) {
                ...
                if (!node.getString("node-id").contains("host")) {
                    String nodedetail = (String) readJsonFromUrl(
                        "http://" + controller.getIp() + ":" + controller.getPort() + "/restconf/operational/opendaylight-inventory:nodes/node/" +
