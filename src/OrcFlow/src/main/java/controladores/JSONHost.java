...
public class JSONHost {
    ...
    throws URISyntaxException {
        ...
        JSONObject arrayHosts;
        JSONArray arrayAttach;
        ...
        try {
            ...
            arrayHosts = new JSONObject(hosts);

            JSONArray arrayDebug = new JSONArray(arrayHosts.get("devices").toString());


            for (int i = 0; i < arrayDebug.length(); i++) {
                host = arrayDebug.getJSONObject(i);
                ...
                if (host.get("ipv4").toString().contains(".")) {
                    ...
                    hostD.setSwitchDPID((String) attach.get("switch").toString());
                    hostD.setPort((Integer) Integer.parseInt(attach.get("port").toString()));
    ...
    throws URISyntaxException {
        ...
        try {
            String topo = readJsonFromUrl("http://" + controller.getIp() + ":" + controller.getPort() + "/restconf/operational/network-topology:network-topology");
